...
A Claude Code plugin that turns a phone on your desk into a live dashboard: your 5-hour, weekly and Fable limits, every active Claude Code session (model, effort, context, what it's doing), and Clawd, Claude's mascot, as a companion that reacts to it all.
...
The companion is Clawd, Claude's mascot, drawn live as an SVG rig (`src/web/clawd/`) with 26 animations that react to Claude Code: thinking, reading, working and compiling while the session he follows is busy, an alert when it needs you, a happy moment when it's your turn, moods as your limits run down, and a calm idle life of blinks, glances, short walks and hops. After a few quiet minutes he yawns and sleeps; tap him to say hi.
...
node tools/clawd-look.mjs <names|all>      # contact sheets of Clawd's animations, as PNGs (in <os tmp>/clawd-look)
node tools/clawd-look.mjs --icon 256 --out src/web   # regenerate the Home Screen icon
```

`src/web/clawd/mock.html` (served at `/web/clawd/mock.html`, e.g. `http://localhost:<port>/web/clawd/mock.html`) plays every animation and the mood engine's sequences in a browser. `src/web/clawd/ANIMATING.md` is the guide to writing animations.
...
- The companion's state machine (`src/web/mood.js`) is adapted from [clawdio](https://github.com/cegware/clawdio) (© Anthrive3D, MIT).
